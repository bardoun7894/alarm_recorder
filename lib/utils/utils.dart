...
reminderDateTime(id,imageString,title,description,payload,context,bool edit,bool camera,bool location)async{
  int hour=0;
  int day=0;
  int minute=0;
...
ifDateAndTimeCancel(String payload,context,id,imageString,title,description,camera,location){
  if(payload=="note"){
    Note note = Note(id: id,imagePath: imageString,title: title,description: description);
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context){return AddNotes(true, camera, location,note: note);}));
...
Future<bool> saveNoteDialog(int id,bool edit ,String descriptionControllertext,String imgString,context,bool camera ,bool location)async {
  String note="note";
  return showDialog(
      context: context,
...
void saveRecord(String payload,context,String nameRecord) async {
  ...
  reminderDateTime( id,"","record",nameRecord,payload,context,null,null,null);
}
void saveNote(int id,bool edit ,String descriptionControllertext,String imgString,context,bool camera,bool location) async {
    String titleData = descriptionControllertext.length > 12 ? descriptionControllertext.substring(0, 12) : descriptionControllertext;
    ...
    reminderDateTime(id,imgString,titleData,descriptionData,"note",context,edit,camera,location);
}
